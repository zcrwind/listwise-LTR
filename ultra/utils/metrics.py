...
# tf.disable_v2_behavior()    # zcr (for shielding the warning)
...
def make_ranking_metric_fn(metric_key,
    ...
    assert metric_key in metric_fn_dict, ('metric_key {} not supported.'.format(metric_key))
...
def _safe_div(numerator, denominator, name='safe_div'):
    ...
    return array_ops.where(
        ...
        # tf.div(numerator, denominator), # --> 未来的版本中将弃用
...
def mean_reciprocal_rank(labels, predictions, weights=None, name=None):
    ...
    with ops.name_scope(name, 'mean_reciprocal_rank',
        ...
        relevance = tf.cast(math_ops.greater_equal(sorted_labels, 1.0), tf.float32)
        # reciprocal_rank = 1.0 / math_ops.to_float(math_ops.range(1, topn + 1))
        reciprocal_rank = 1.0 / tf.cast(math_ops.range(1, topn + 1), tf.float32)
...
def expected_reciprocal_rank(
    ...
    with ops.name_scope(name, 'expected_reciprocal_rank',
        ...
        reciprocal_rank = 1.0 / tf.cast(math_ops.range(1, list_size + 1), tf.float32)
        # mask = math_ops.to_float(math_ops.greater_equal(reciprocal_rank, 1.0 / (topn + 1)))
        mask = tf.cast(math_ops.greater_equal(reciprocal_rank, 1.0 / (topn + 1)), tf.float32)
...
def average_relevance_position(labels, predictions, weights=None, name=None):
    ...
    with ops.name_scope(name, 'average_relevance_position',
        ...
        position = tf.cast(math_ops.range(1, topn + 1), tf.float32)
...
def precision(labels, predictions, weights=None, topn=None, name=None):
    ...
    with ops.name_scope(name, 'precision', (labels, predictions, weights)):
        ...
        per_list_weights = _per_example_weights_to_per_list_weights(
            ...
            weights, tf.cast(math_ops.greater_equal(labels, 1.0)), tf.float32)
...
def normalized_discounted_cumulative_gain(labels,
    ...
    with ops.name_scope(name, 'normalized_discounted_cumulative_gain',
        ...
        per_list_weights = _per_example_weights_to_per_list_weights(
            ...
            relevance=math_ops.pow(2.0, tf.cast(labels, tf.float32)) - 1.0)
...
def ordered_pair_accuracy(labels, predictions, weights=None, name=None):
    ...
    with ops.name_scope(name, 'ordered_pair_accuracy',
        ...
        correct_pairs = tf.cast(pair_label_diff > 0, tf.float32) * tf.cast(pair_pred_diff > 0, tf.float32)
        # pair_weights = math_ops.to_float(pair_label_diff > 0) * array_ops.expand_dims(
        #         weights, 2) * math_ops.to_float(valid_pair)
        pair_weights = tf.cast(pair_label_diff > 0, tf.float32) * array_ops.expand_dims(
