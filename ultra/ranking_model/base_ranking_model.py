...
class BaseRankingModel(ABC):
    ...
    def get_variable(self, name, shape, noisy_params=None, noise_rate=0.05, **kwargs):
        ...
        Returns:
            A tf.Tensor
        """
        # tf.get_variable_scope().reuse_variables() # useless for the bug `ValueError: Variable dnn_W_0 does not exist, or was not created with tf.get_variable(). Did you mean to set reuse=tf.AUTO_REUSE in VarScope?`
        var = tf.get_variable(name, shape, **kwargs)    # tf1
        # var = tf.Variable(name, shape, **kwargs)    # tf2
        # cprint('var.dtype: {}'.format(var.dtype), 'yellow') # <dtype: 'string'>
        # # cprint('var: {}'.format(var), 'yellow')
        # cprint('>' * 20, 'yellow')
        # tf.print(var)
        # cprint('<' * 30, 'yellow')
