...
# from tensorflow.contrib.rnn.python.ops import core_rnn_cell # error: ModuleNotFoundError: No module named 'tensorflow.contrib'
# from tensorflow.keras.layers import LSTMCell as core_rnn_cell   # zcr --> 并不是等效上面一行代码：上一行代码在该文件中存在的意义是用core_rnn_cell._linear, 其实是做W*X+b的
'''
直接在tf2上修改的方式：
...
# linear_w = core_rnn_cell # zcr
# from tensorflow.layers import Linear
# linear = Linear(use_bias=True)
sys.path.append('../..')
from utils.temp_module import _linear as linear
...
#         self.embeddings = tf.placeholder(tf.float32, shape=[None, embed_size], name="embeddings")
    ...
    def rnn_decoder(self, encode_embed, attention_states, initial_state, cell,
        ...
        with tf.variable_scope(scope or "rnn_decoder"):
            ...
            # reshape before.
            print(attention_states.get_shape(), "attention_states.get_shape()") # (?, 9, 186)
            ...
            for a in xrange(num_heads):
                ...
                u.append(self.get_variable("AttnU_%d" % a,
                                           [attention_vec_size]))
                head_weights.append(self.get_variable("head_weight_%d" % a, [1]))
            ...
            def attention(query):
                ...
                print(hidden.get_shape(), "hidden.get_shape()") # (?, 9, 1, 186)
                print(tiled_query.get_shape(), "tiled_query.get_shape()")   # (?, 9, 1, 186)
                ...
                for a in xrange(num_heads):
                    with tf.variable_scope("Attention_%d" % a):
                        ...
                        if self.hparams.att_strategy == 'multi':
                            ...
                            y = linear(query, attention_vec_size, True) # 第三个参数是boolean, whether to add a bias term or not.
