...
class DNN(BaseRankingModel):
    ...
    def build(self, input_list, noisy_params=None,
        ...
        # cprint('tf.get_variable_scope().name: {}'.format(tf.get_variable_scope().name), 'red')   # 输出是空字符串, 说明是全局变量范围
        # cprint('tf.get_variable_scope().name: {}'.format(tf.get_variable_scope().reuse), 'red')   # True, 是可以复用变量的
        with tf.variable_scope(tf.get_variable_scope(), initializer=self.initializer, reuse=tf.AUTO_REUSE):
            input_data = tf.concat(input_list, axis=0)
            output_data = input_data
            output_sizes = self.hparams.hidden_layer_sizes + [1]
            ...
            if self.layer_norm is None and self.hparams.norm in BaseRankingModel.NORM_FUNC_DIC:
                self.layer_norm = []
                for j in range(len(output_sizes)):
                    self.layer_norm.append(BaseRankingModel.NORM_FUNC_DIC[self.hparams.norm](name="layer_norm_%d" % j))
            ...
            for j in range(len(output_sizes)):
                ...
                expand_W = self.get_variable("dnn_W_%d" % j, [current_size, output_sizes[j]],
                                             noisy_params=noisy_params, noise_rate=noise_rate)
                expand_b = self.get_variable("dnn_b_%d" % j, [output_sizes[j]],
                                             noisy_params=noisy_params, noise_rate=noise_rate)
                
                # cprint('type(output_data): {}'.format(type(output_data)), 'red')    # <class 'tensorflow.python.framework.ops.Tensor'>
                # cprint('type(expand_W): {}'.format(type(expand_W)), 'red')  # <class 'tensorflow.python.ops.resource_variable_ops.ResourceVariable'>
                # cprint('output_data.dtype: {}'.format(output_data.dtype), 'yellow')     # <dtype: 'float32'>
                # cprint('expand_W.dtype: {}'.format(expand_W.dtype), 'yellow')   # 用tf2的tf.Variable: <dtype: 'string'>; 用tf1的tf.get_varialbe: <dtype: 'float32_ref'>
                # cprint('expand_b.dtype: {}'.format(expand_b.dtype), 'yellow')   # 用tf2的tf.Variable: <dtype: 'string'>; 用tf1的tf.get_varialbe: <dtype: 'float32_ref'>
