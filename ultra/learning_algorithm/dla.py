...
class DLA(BaseAlgorithm):
    ...
    * Qingyao Ai, Keping Bi, Cheng Luo, Jiafeng Guo, W. Bruce Croft. 2018. Unbiased Learning to Rank with Unbiased Propensity Estimation. In Proceedings of SIGIR '18
    ...
    def __init__(self, data_set, exp_settings, forward_only=False):
        super(DLA, self).__init__(data_set, exp_settings, forward_only) # zcr
        """Create the model.
        ...
        # cprint(exp_settings['learning_algorithm_hparams'], 'blue')
        ...
        if self.hparams.propensity_learning_rate < 0:
            self.propensity_learning_rate = tf.Variable(float(self.hparams.learning_rate), trainable=False)
        else:
            self.propensity_learning_rate = tf.Variable(float(self.hparams.propensity_learning_rate), trainable=False)
        ...
        for i in range(self.max_candidate_num):
            self.docid_inputs.append(tf.placeholder(tf.int64, shape=[None], name="docid_input{0}".format(i)))
            self.labels.append(tf.placeholder(tf.float32, shape=[None], name="label{0}".format(i)))
        ...
        if not forward_only:    # training mode
            # Build model
            ...
            for i in range(len(pw_list)):
                tf.summary.scalar('Inverse_Propensity_weights_%d' % i, tf.reduce_mean(pw_list[i]), collections=['train'])
            tf.summary.scalar('Rank_Loss', tf.reduce_mean(self.rank_loss), collections=['train'])
            ...
            for i in range(len(rw_list)):
                tf.summary.scalar(
                    'Relevance_weights_%d' %
                    i, tf.reduce_mean(rw_list[i]), collections=['train'])
            tf.summary.scalar('Exam_Loss', tf.reduce_mean(self.exam_loss), collections=['train'])
            ...
            tf.summary.scalar('Gradient_Norm', self.norm, collections=['train'])
            tf.summary.scalar('Learning_Rate', self.learning_rate, collections=['train'])
            tf.summary.scalar('Final_Loss', tf.reduce_mean(self.loss), collections=['train'])
    ...
    def DenoisingNet(self, list_size, forward_only=False, scope=None):
        ...
        with tf.variable_scope(scope or "denoising_model", reuse=tf.AUTO_REUSE):    # zcr加上了reuse=tf.AUTO_REUSE
