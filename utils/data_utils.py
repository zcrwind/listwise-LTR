...
class Raw_data:
    def __init__(self, data_path=None, file_prefix=None, rank_cut=None):
        ...
        ULTRA_judge_path = os.path.join(data_path, file_prefix, file_prefix + '.feature')
        cprint(ULTRA_judge_path, 'green')
        libsvm_judge_path = os.path.join(data_path, file_prefix, file_prefix + '.txt')
        cprint(libsvm_judge_path, 'green')

        if os.path.isfile(ULTRA_judge_path):  # files in ULTRA data format
    ...
    def load_data_in_ULTRA_format(
        ...
        print('Read data from {} in ULTRA format.'.format(os.path.join(data_path, file_prefix)))
    ...
    def load_data_in_libsvm_format(
        ...
        Returns:
            None
        """
        print('Read data from {} in libsvm format.'.format(os.path.join(data_path, file_prefix)))
        self.load_basic_data_information(data_path)

        # cprint(data_path + file_prefix + '/' + file_prefix + '.txt', 'blue')
        input_path = os.path.join(data_path, file_prefix, file_prefix + '.txt')
        feature_fin = open(input_path)
        qid_to_idx = {}
        line_num = -1
        # line_num = 0   # zcr
        for line in feature_fin:
